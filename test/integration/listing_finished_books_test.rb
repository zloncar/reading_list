...
class ListingFinishedBooksTest < ActionDispatch::IntegrationTest
  ...
  setup do
    ...
    Book.create!(title: 'Also Finished', finished_at: 1.day.ago)
  ...
  test 'list finished books in JSON' do
    ...
    assert_equal 2, json(response.body)[:finished_books].size
  end
  ...
  test 'list finished books in XML' do
    ...
    assert_equal 2, Hash.from_xml(response.body)['books'].size
  end
end
